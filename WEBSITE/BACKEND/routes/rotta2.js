...
const Logicabusiness2 =  require('../libs/logicabusiness2');
...
router
    ...
    .post('/continuoRotta/:id',Middleware(100,0),(req,res) => { //Se nell'index iniziava per /rotta2, quando entra qua dentro diventa /rotta2/continuoRotta/idchemettetedalfrontend quindi lo user dovra accedere a http://dominio:porta/rotta2/continuoRotta/idchemettetedalfrontend e potra entrare qua dentro; altro esempio piu concreto  http://dominio:porta/rotta2/continuoRotta/127642810/
    ...
    Logicabusiness2.getPerId(req.params.id).then((risposta) => {
        if(!risposta)
    ...
    .get('/continuoRottaDue/:id',Middleware(199,0),async (req,res) => { //STESSO ESEMPIO DI SOPRA NEL CASO NON VOGLIO USARE UNA THEN MA UN AWAIT/ASYNC, FANNO LA STESSA E IDENTICA COSA
        try{
        let risposta = await Logicabusiness2.getPerId(req.params.id);
        if(!risposta)
            return res.status(404).json({message: "Risposta not founded!"}); //RITORNO AL FRONTENT UNA RISPOSTA DI ERRORE CON CODICE 404
        return res.status(200).json(risposta); //INVIO I DATI CON CODICE 200 (ovvero tutt ok)
        //RISPOSTA SARA CONVERTITO DA OGGETTO JAVASCRIPT A JSON PER IL FRONTEND
